...
export function getHover(workspace: Workspace, uri: string, dialect: Dialect, word: string, line = 0): Hover | undefined {
  ...
  // A definition written in *this* file outranks the built-in tables, on the same reasoning as the
  // two carve-outs above: the user's own `macro mov` wrapper (a common idiom in macro-heavy fasm
  // code) is unambiguously what they meant, yet it was permanently shadowed by x86's entry for the
  // same spelling. Restricted to the current document on purpose — merely making an x86 package
  // reachable through `includePath` would otherwise replace the curated x86 prose with a bare
  // "Macro" for every mnemonic, since fasmg implements them all as `calminstruction mov?` etc.
  const definedHere = currentDoc?.symbols.find((s) => s.name === word && !s.localScope);
  if (definedHere) return markdown(renderSymbol(definedHere, uri, false));

  // The instruction and register tables describe x86 only. When the include graph supplies its own
  // instruction set, they are not merely unhelpful but wrong — answering an aarch64 file's `mov`
  // with x86's meaning — so they are skipped and the include-graph walk below answers instead.
  const isa = detectIsa(workspace, uri, dialect);

  const ins = isa === 'x86' ? instructionsByMnemonic.get(lower) : undefined;
  if (ins) return markdown(renderInstructions(ins));

  const reg = isa === 'x86' ? registersByName.get(lower)?.[0] : undefined;
...
const DEFINED_VIA_RENDER: Record<NonNullable<SymbolDefinition['definedVia']>, { syntax: (name: string, value: string) => string; note?: string }> = {
  ...
  element: {
    syntax: (n, v) => (v ? `element ${n} : ${v}` : `element ${n}`),
    note: 'A symbolic term, not a plain number — it keeps its own identity in an expression, which is how instruction-set packages declare register names (fasmg\'s aarch64 package declares every register this way) so that an instruction can tell `x0` from the value 0.',
  },
  'struct-size': {
    syntax: (n) => n,
    note: 'Auto-generated by the `struct` package for every struct (macro/struct.inc) — equals the total byte size of the struct, usable directly in an expression (e.g. "add esi, sizeof.MatchedExcerpt") without instantiating one. See the struct\'s own definition (jumped to by go-to-definition on this symbol) for its fields.',
  },
};
