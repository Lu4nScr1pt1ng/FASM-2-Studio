...
describe('getHover', () => {
  ...
  describe('user symbols', () => {
    ...
    it('hovers a fully-qualified struct field ("StructName.field") the same as a real code site would reference it', async () => {
      // Real user-reported scenario: fasmg's struct package (macro/struct.inc) wraps a struct's
      // whole body in "namespace <name>", so real code always references a field fully qualified
      // (e.g. "[ebx+MatchedExcerpt.matcher]") -- hovering that exact token found nothing at all
      // before this, since only the bare field name was ever indexed.
      const src = ['format binary', 'struct MatchedExcerpt', '\tmatcher dd ?', 'ends'].join('\n');
      const mainUri = await writeFile('main.asm', src);
      const local = new Workspace();
      local.updateDocument(mainUri, 1, src, 'fasm2');

      const v = value(getHover(local, mainUri, 'fasm2', 'MatchedExcerpt.matcher'));
      assert.match(v, /\*\*MatchedExcerpt\.matcher\*\* — \*Label\*/);
    });

    it('hovers "sizeof.<StructName>" as the struct package\'s own auto-generated size constant, not nothing at all', async () => {
      // The other half of the same real bug: fasmg's struct package auto-generates a
      // "sizeof.<StructName>" companion constant when a struct closes (confirmed against the real
      // struct.inc source), used directly in real code (e.g. "add esi, sizeof.RecognitionContext")
      // -- nothing indexed this before, so hovering it found nothing.
      const src = ['format binary', 'struct RecognitionContext', '\tbase_namespace dd ?', 'ends'].join('\n');
      const mainUri = await writeFile('main.asm', src);
      const local = new Workspace();
      local.updateDocument(mainUri, 1, src, 'fasm2');

      const v = value(getHover(local, mainUri, 'fasm2', 'sizeof.RecognitionContext'));
      assert.match(v, /```fasm\nsizeof\.RecognitionContext\n```/);
      assert.match(v, /\*Constant\*/);
      assert.match(v, /Auto-generated by the `struct` package/);
    });

    it('hovers a struct instance and its fields through "instanceName.field", not just the struct type name directly', async () => {
      // Real user-reported follow-up: "assembly_workspace Workspace" declares an *instance* of
      // struct Workspace (fasmg's struct package dynamically generates a command literally named
      // after the struct once it's defined) -- real code references its fields through the
      // instance ("assembly_workspace.memory_start"), not through the struct type name.
      const src = ['format binary', 'struct Workspace', '\tmemory_start dd ?', '\tmemory_end dd ?', 'ends', '', 'assembly_workspace Workspace'].join('\n');
      const mainUri = await writeFile('main.asm', src);
      const local = new Workspace();
      local.updateDocument(mainUri, 1, src, 'fasm2');

      const fieldHover = value(getHover(local, mainUri, 'fasm2', 'assembly_workspace.memory_start'));
      assert.match(fieldHover, /\*\*Workspace\.memory_start\*\* — \*Label\*/);

      const instanceHover = value(getHover(local, mainUri, 'fasm2', 'assembly_workspace'));
      assert.match(instanceHover, /Instance of struct `Workspace`/);
    });

    it('does not misread an ordinary macro invocation with one bare argument as a struct instance', async () => {
      // Same "IDENT1 IDENT2" shape as a real instantiation -- must not resolve to anything just
      // because a macro happens to take exactly one bare-identifier argument.
      const src = ['format binary', 'macro write_msg target', '\tcall target', 'end macro', '', 'write_msg usage_text'].join('\n');
      const mainUri = await writeFile('main.asm', src);
      const local = new Workspace();
      local.updateDocument(mainUri, 1, src, 'fasm2');

      assert.strictEqual(getHover(local, mainUri, 'fasm2', 'write_msg.anything'), undefined);
    });
  ...
  describe('when the include graph supplies a non-x86 instruction set', () => {
    ...
    it("resolves a mnemonic spelled like an x86 one to the package's own macro, not to x86", async () => {
      const { ws, uri } = await foreignIsaProject();
      const v = value(getHover(ws, uri, 'fasm2', 'mov'));

      // The x86 table was consulted before the include-graph walk, so this used to answer with
      // x86's "Copy a value between registers, memory and immediates" for every one of the 23
      // mnemonics the two instruction sets happen to spell the same way.
      assert.ok(v.includes('Macro'), `expected the package's own macro, got: ${v}`);
      assert.ok(!v.includes('x86 instruction'), 'must not describe an x86 instruction in a non-x86 file');
    });

    it('resolves a register generated by `repeat`/`element` in the package', async () => {
      const { ws, uri } = await foreignIsaProject();
      const v = value(getHover(ws, uri, 'fasm2', 'x0'));
      assert.match(v, /myreg/);
    });

    it('gives no x86 register answer for a name x86 happens to define', async () => {
      const { ws, uri } = await foreignIsaProject();
      // "al" is a real x86 register and means nothing in this package.
      const h = getHover(ws, uri, 'fasm2', 'al');
      assert.ok(!h || !value(h).includes('register'), 'must not describe an x86 register in a non-x86 file');
    });

    it('still explains fasmg directives, which are engine syntax and ISA-independent', async () => {
      const { ws, uri } = await foreignIsaProject();
      assert.ok(getHover(ws, uri, 'fasm2', 'virtual'), 'expected the core directive to still resolve');
    });
  });

  describe('a definition in the current document', () => {
    it('outranks the built-in x86 table for the same spelling', () => {
      const ws = new Workspace();
      const uri = 'file:///wrapper.asm';
      // Wrapping an instruction in a same-named macro is a common idiom in macro-heavy fasm code;
      // the user's own definition is unambiguously what they meant when they hover it.
      ws.updateDocument(uri, 1, 'macro mov dest, src\n\tstore dest, src\nend macro\n', 'fasm2');

      const v = value(getHover(ws, uri, 'fasm2', 'mov'));
      assert.ok(v.includes('Macro'), `expected the local macro, got: ${v}`);
      assert.ok(!v.includes('x86 instruction'));
    });

    it('does not shadow x86 prose for a mnemonic the document does not define', () => {
      const ws = new Workspace();
      const uri = 'file:///plain.asm';
      ws.updateDocument(uri, 1, 'macro mov dest, src\nend macro\n', 'fasm2');

      const v = value(getHover(ws, uri, 'fasm2', 'add'));
